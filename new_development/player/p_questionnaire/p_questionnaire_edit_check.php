<!-- 
    p_questionnaire_edit.phpからの入力情報をチェックし、遷移先を決定する。
    問題なし→p_questionnaire_edit_done.phpへリダイレクト
    問題あり→エラーメッセージを出力し、戻るボタンでp_questionnaire_edit.phpへ遷移
 -->
...
if ($flg) {             // 入力に問題がなかった場合
    ...
    header('Location:p_questionnaire_edit_done.php');    // p_questionnaire_edit_done.phpへリダイレクト
    exit();
} else {                // 入力に問題があった場合
    print '<br><br>';
    print '<input type="button" onclick="location.href=\'p_questionnaire_edit.php\'" value="戻る">';
}