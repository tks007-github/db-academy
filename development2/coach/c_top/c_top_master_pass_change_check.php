...
try {
    ...
    if ($flg) {             // 入力に問題がなかった場合
        $_SESSION['new_mst_password'] = $new_mst_password;          // セッション変数にnew_mst_passwordを保持
        header('Location:c_top_master_pass_change_done.php');       // c_top_master_pass_change_done.phpへリダイレクト
        exit();
    } else {                // 入力に問題があった場合
        print '<br>';
        print '<input type="button" onclick="location.href=\'c_top_master_pass_change.php\'" value="戻る">';
    }
